    ...
    <>
      <li>
        <Link
          href={item.route}
          onClick={handleClick}
          className={`${pageName === item.label.toLowerCase() ? 'bg-locaGreen text-white' : 'text-dark-6 hover:bg-gray-2 hover:text-dark '} group relative flex items-center gap-2.5 rounded-[7px] px-3.5 py-3 font-medium duration-300 ease-in-out text-sm`}
        >
          <span className={pageName === item.label.toLowerCase() ? 'active' : ''}>{item.icon}</span>
