    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative hidden sm:block">
      <li style={{ listStyle: 'none' }}>
        <Link
          ...
          href="#"
          className="relative flex h-12 w-12 items-center justify-center rounded-full bg-gray-2 text-dark-5"
        >
          <span className="relative">
            ...
            <span
              className={`absolute -top-0.5 right-0 z-1 h-2.5 w-2.5 rounded-full border-2 border-gray-2 bg-red-500 ${
        ...
        {dropdownOpen && (
          <div
            className={`absolute -right-27 mt-7.5 flex h-[550px] w-75 flex-col rounded-xl border-[0.5px] border-stroke bg-white px-5.5 pb-5.5 pt-5 shadow-default sm:right-0 sm:w-[364px]`}
          >
            <div className="mb-5 flex items-center justify-between px-3">
              <h5 className="text-lg font-medium text-dark ">Notifications</h5>
              <span className="rounded-md bg-locaGreen px-3 py-1.5 text-xs font-medium text-white">5 new</span>
            </div>

            <ul className="no-scrollbar mb-5 flex h-auto flex-col gap-1 overflow-y-auto px-3">
              {notificationList.map((item, index) => (
                <li key={index}>
                  <Link className="flex items-center gap-4 rounded-[10px] p-2.5 hover:bg-gray-2" href="#">
                    ...
                    {/* <span className="block">
                      <p className="block font-medium text-dark ">
                        {item.title}
                      </p>
                      <p className="block text-body-sm font-medium text-dark-5 ">
                        {item.subTitle}
                      </p>
                    </span> */}
                    <div className="flex flex-col justify-center gap-0">
                      <p className="text-sm font-medium text-dark">{item.title}</p>
                      <p className="text-xs text-dark-5">{item.subTitle}</p>
                    </div>
                  </Link>
                </li>
              ))}
            ...
            <Link
              className="flex items-center justify-center rounded-[7px] border border-locaGreen mx-5 font-medium text-locaGreen hover:bg-blue-light-5 h-10"
              href="#"
            >
